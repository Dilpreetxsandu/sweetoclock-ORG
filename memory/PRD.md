...
- PENDING from user: (1) Shiprocket pickup street address + contact phone (account has zero pickup locations — shipments can't be created until registered). (2) AiSensy: API key added 2026-09-15 and endpoint verified — AiSensy FULLY LIVE (2026-09-18): API campaign "Order Confirmation" (template `orderconfirmation`, PDF header) verified end-to-end — success:true, submitted_message_id returned. Invoices now send on WhatsApp at payment + admin resend.
